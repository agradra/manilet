...
def _enforce_single_instance(port: int = 58291):
    ...
    try:
        ...
        print("\r\033[2K\033[38;2;255;60;60mNexus가 이미 실행 중입니다.\033[0m")
        sys.exit(0)
...
def _color_print(
    text, red: int = 255, green: int = 255, blue: int = 255, end: str | None = "\n", flush: bool = False
) -> None | str:
...
def main():
    ...
    try:
        ...
        if sys.version_info < (3, 12):
            print("\033[2K\033[38;2;255;255;60m파이썬 3.12 이상 버전이 필요합니다.\033[0m", end="", flush=True)
        ...
        if is_password_setting():  # 이미 비번 있음
            while True:
                ...
                if CryptoDB.initialize(pw):
                    _color_print(
                        "\033[2K\r\033[1A\033[2K\rPassword verification completed", 100, 255, 100, end="", flush=True
                    )
                    break
                else:
            ...
            _color_print(
                "비밀번호를 생성하세요. 비밀번호를 분실할 경우 저장된 데이터는 절대 복구할 수 없습니다.", 255, 255, 60
            )
            while True:
                ...
                if pw1 == pw2:
                    ...
                    _color_print(
                        "\033[2K\r\033[1A\033[2K\r\033[2K\r\033[1A\033[2K\rPasswords do not match", 255, 100, 100
                    )
            if not CryptoDB.initialize(pw1):
                print("\033[2K\033[38;2;255;255;60m초기 db세팅인데 비번이 틀렸다는디??\033[0m", end="", flush=True)
                return
