...
def _build_discord_payload(data: dict[str, list[Log]]) -> dict:
    ...
    for sender, logs in list(data.items())[:10]:
        ...
        embed = {"color": color, "description": description, "fields": []}
        ...
        for log in logs[:25]:
            ...
            if len(safe_msg) > 1000:
                safe_msg = safe_msg[:995] + "..."  # value 1024 초과시 서버 안받음
...
class Discord(metaclass=Singleton):
    ...
    def _status_loop(self):
        ...
        while True:
            try:
                ...
                if fail_n >= 3:
                    if fail_n < 10:
                        self._sys_logger.emit(
                            LogLevel.WARN,
                            f"디스코드 서버 상태 모니터링 요청 실패 ({fail_n}번째 요청 실패, 30초간 일시정지) - {e}",
                        )
                        time.sleep(30)
                        ...
                        self._sys_logger.emit(
                            LogLevel.ERROR,
                            f"디스코드 서버 상태 모니터링 10번째 요청 실패, 모드 변경( {old_mode} → {new_mode} )",
                        )
    ...
    def _www_send_log(self, is_test: bool = False):
        ...
        try:
            ...
            with self._lock:
                self.is_connect = False
            self._sys_logger.emit(LogLevel.WARN, "디스코드 로그 전송 실패 (5초 후 실패 로그 재전송 시도)")
            time.sleep(5)
            res = requests.post(target_url, json=data, timeout=5)
            res.raise_for_status()  # 200번대 응답 아니면 에러 발생
            with self._lock:
    ...
    def _www_update_status(self) -> None:
        ...
        if self._status_md_fn is None:
            raise RuntimeError(
                "서버 상태 모니터링 업데이트용 Markdown 생성 함수가 설정되지 않았습니다 - ( _status_md_fn = None )"
            )
        ...
        if self._status_msg_id is None:  # 최초 POST
            ...
            if res.status_code in (200, 204):
                ...
                self._sys_logger.emit(
                    LogLevel.WARN, "디스코드 서버 상태 모니터링 삭제 감지 - 재생성 시도 및 진행 (404)"
                )
